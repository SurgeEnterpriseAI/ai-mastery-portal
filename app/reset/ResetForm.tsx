...
export default function ResetForm({ token }: { token: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    const res = await fetch("/api/learner/reset", {
      method: "POST",
    ...
    if (res.ok) {
      setDone(true);
      setTimeout(() => { router.push("/learn"); router.refresh(); }, 1200);
    } else {
  ...
  if (done) {
    return <div className="mt-6 rounded-lg bg-emerald-50 p-4 text-sm text-emerald-800">✓ Password updated — signing you in…</div>;
  }

      ...
      <section>
        ...
        <form onSubmit={createCohort} className="mt-3 grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Class time</span>
            <TimeRangePicker value={classTime} onChange={setClassTime} />
          </div>
...
function CohortTime({ cohort, onSaved }: { cohort: Cohort; onSaved: () => void }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2">
      ...
      <TimeRangePicker value={t} onChange={setT} />
...
// 30-minute slots across the day, shown in 12-hour format.
...
const TZS = ["IST", "GMT", "GST", "SGT", "AEST", "CET", "EST", "PST"];
const labelOf = (v: string) => SLOTS.find((s) => s.value === v)?.label || "";
const valueOf = (lab: string) => SLOTS.find((s) => s.label === lab)?.value || "";
function parseRange(v: string) {
  const m = (v || "").match(/^(.+?)\s*[–-]\s*(.+?)\s+([A-Za-z]+)$/);
  if (!m) return { start: "", end: "", tz: "IST" };
  return { start: valueOf(m[1].trim()), end: valueOf(m[2].trim()), tz: m[3] || "IST" };
}

// Start/end time pickers in 30-min steps + timezone; composes a "7:00 PM – 8:30 PM IST" string.
function TimeRangePicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const init = parseRange(value);
  const [start, setStart] = useState(init.start);
  const [end, setEnd] = useState(init.end);
  const [tz, setTz] = useState(init.tz || "IST");
  const sel = "rounded-lg border border-slate-300 bg-white px-2 py-2 text-sm text-slate-900 outline-none focus:border-brand-500";
  function emit(s: string, e: string, z: string) {
    onChange(s && e ? `${labelOf(s)} – ${labelOf(e)} ${z}` : "");
  }
  return (
    <div className="mt-1 flex flex-wrap items-center gap-2">
      <select value={start} onChange={(e) => { setStart(e.target.value); emit(e.target.value, end, tz); }} className={sel}>
        <option value="">Start…</option>
        {SLOTS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
      </select>
      <span className="text-sm text-slate-400">to</span>
      <select value={end} onChange={(e) => { setEnd(e.target.value); emit(start, e.target.value, tz); }} className={sel}>
        <option value="">End…</option>
        {SLOTS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
      </select>
      <select value={tz} onChange={(e) => { setTz(e.target.value); emit(start, end, e.target.value); }} className={sel}>
